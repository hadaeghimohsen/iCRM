...
BEGIN
   ...
   WHEN MATCHED THEN
      UPDATE 
         SET MDFY_BY        = UPPER(SUSER_NAME())
            ...
            ,DEBT_DNRM      = (SELECT DEBT_DNRM FROM Service WHERE RQST_RQID = T.RQRO_RQST_RQID)
            ...
            --,DCMT_PRIC      = CASE WHEN S.DCMT_PRIC IS NULL THEN (SELECT SUM_RCPT_EXPN_PRIC + ISNULL(SUM_RCPT_EXPN_EXTR_PRCT, 0) + ISNULL(SUM_RCPT_REMN_PRIC, 0) FROM Payment P WHERE P.RQST_RQID = (SELECT RQST_RQID FROM Request WHERE RQID = T.RQRO_RQST_RQID)) - (SELECT DEBT_DNRM FROM Service WHERE RQST_RQID = T.RQRO_RQST_RQID) ELSE S.DCMT_PRIC END
            /*,DCMT_DESC      = CASE WHEN S.DCMT_DESC IS NULL THEN (SELECT N'استرداد وجه ' + (P.SUM_RCPT_EXPN_PRIC + ISNULL(P.SUM_RCPT_EXPN_EXTR_PRCT, 0) + ISNULL(P.SUM_RCPT_REMN_PRIC, 0)) + N' ریال بابت درخواست' + Rt.RQTP_DESC + N' به ' + CASE F.SEX_TYPE_DNRM WHEN '001' THEN N' آقای ' ELSE N' سرکارخانم ' END + F.NAME_DNRM + N' برگشت داده شد!'
                                                                    FROM Request_Type Rt, Request R, Request_Row Rr, Service F, Payment P
	...
	BEGIN
	   ...
	   SELECT @RegnPrvnCntyCode = F.REGN_PRVN_CNTY_CODE
	         ...
	         ,@COMPCode = COMP_CODE_DNRM
	         ,@ExpnAmnt = I.DCMT_PRIC
	         ,@Rqid = F.RQST_RQID	         
	         ,@FileNo = F.FILE_NO
	     FROM dbo.Request_Row Rr, Service F, Inserted I
	    WHERE Rr.RQST_RQID = I.RQRO_RQST_RQID
	      AND Rr.RWNO = I.RQRO_RWNO
	      AND Rr.SERV_FILE_NO = F.FILE_NO;

	   SET @ActnDate = GETDATE();
	   EXEC dbo.INS_ACTN_P @RegnPrvnCntyCode, @RegnPrvnCode, @RegnCode, @COMPCode, 0, '001', @ActnDate, @Rwno OUT;
	   EXEC dbo.INS_ACDT_P @RegnPrvnCntyCode, @RegnPrvnCode, @RegnCode, @COMPCode, @Rwno, @ExpnAmnt, '001', @ActnDate, NULL, @Rqid, NULL, @AcntRwno OUT;
	END
   ...
   BEGIN
      ...
      SELECT @Rqid = I.RQRO_RQST_RQID
	     ...
	     FROM Request_Row Rr, Inserted I, dbo.Service f
	    WHERE Rr.RQST_RQID = I.Rqro_Rqst_Rqid
	      AND Rr.RWNO = I.Rqro_Rwno
	      AND rr.SERV_FILE_NO = f.FILE_NO;	    
      
      EXEC dbo.INS_GLRL_P @RqroRqstRqid = @Rqid, @RqroRwno = @Rwno, @SERVFileNo = @FileNo, @ConfStat = '002', @ChngType = '002', @DebtType = '003', @Amnt = @Amnt, @AgreDate = @AgreDate, @PaidDate = @PaidDate, @ChngResn = '005', @ResnDesc = N'برگشت هزینه به صورت اتوماتیک پیرو درخواست', @Glid = @Glid OUT;                   
   END
