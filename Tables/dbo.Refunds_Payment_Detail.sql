...
[PYDT_RCPT_BTRF] [varchar] (3) COLLATE SQL_Latin1_General_CP1_CI_AS NULL,
...
BEGIN
	...
	DECLARE C$AINS_RFPD CURSOR FOR
	   SELECT FIDC_FDID, PYDT_CODE, PYDT_EXPN_CODE, RFID, PYDT_PAY_STAT,
	          ...
	          PYDT_QNTY, PYDT_DOCM_NUMB, PYDT_ISSU_DATE, PYDT_RCPT_BTRF,
   ...
   UPDATE dbo.Refunds_Payment_Detail
	...
	UPDATE dbo.Finance_Document  
	   SET PYMT_PRIC_DNRM = ROUND(ISNULL((
	       ...
	       GET_PYMT_PRIC_DNRM = (
	         SELECT SUM(AMNT)
	           FROM dbo.Payment_Base_Tariff Pm
	          WHERE EXISTS (
	            SELECT *
	              FROM dbo.Finance_Document F, dbo.Request_Row Rr, dbo.Request R, dbo.Payment P, dbo.Payment_Base_Tariff Pm1
