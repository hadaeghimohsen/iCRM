...
SELECT RQTP.CODE AS RQTP_CODE, RQTP.RQTP_DESC, RQTP.MUST_LOCK AS RQTP_MUST_LOCK, RQTT.RQTT_DESC, 
    ...
    EXPN.EXPN_STAT, TRFD.NAME AS TRFD_NAME, TRFD.TRFD_DESC, TRFD.CODE AS TRFD_CODE, 
    BTRF.CODE AS BTRF_CODE, BTRF.BTRF_DESC, SERV.FILE_NO, REGL_ACNT.TYPE AS REGL_ACNT_TYPE, 
...
FROM dbo.Request_Requester AS RQRQ INNER JOIN
    ...
    dbo.Base_Tariff_Detail AS TRFD ON EXPN.TRFD_CODE = TRFD.CODE INNER JOIN
    dbo.Base_Tariff AS BTRF ON EXPN.BTRF_CODE = BTRF.CODE AND TRFD.BTRF_CODE = BTRF.CODE INNER JOIN
    ...
    dbo.Service AS SERV ON REGN.PRVN_CNTY_CODE = SERV.REGN_PRVN_CNTY_CODE AND 
    REGN.PRVN_CODE = SERV.REGN_PRVN_CODE AND REGN.CODE = SERV.REGN_CODE AND 
    TRFD.CODE = SERV.TRFD_CODE_DNRM AND BTRF.CODE = SERV.BTRF_CODE_DNRM INNER JOIN
