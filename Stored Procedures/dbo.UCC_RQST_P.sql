	...
	*/
	...
	BEGIN TRY
	   ...
	   SELECT @RegnCode = REGN_CODE
	     FROM Service WHERE FILE_NO = @FileNo;      
      ...
      BEGIN
         DECLARE @SRPBType VARCHAR(3);
         SELECT @SRPBType = SRPB_TYPE_DNRM
           FROM Service
          WHERE FILE_NO = @FileNo;
      
         IF @SRPBType = '005' --هنرجوی مهمان 
         BEGIN
         ...
         BEGIN
            SET @StrtDate = GETDATE();
            --SET @EndDate  = NULL;
            --SELECT @StrtDate = COALESCE (DATEADD(DAY, 1, END_DATE), GETDATE()) FROM Member_Ship m, Service F WHERE m.SERV_FILE_NO = @FileNo AND m.RECT_CODE = '004' AND F.FILE_NO = m.SERV_FILE_NO AND F.MBSP_RWNO_DNRM = M.RWNO;
            SET @EndDate = DATEADD(MONTH, 1, @StrtDate);
         END
      END
      
      IF NOT EXISTS(SELECT * FROM Member_Ship WHERE RQRO_RQST_RQID = @Rqid AND RQRO_RWNO = @RqroRwno AND SERV_FILE_NO = @FileNo AND RECT_CODE = '001')
      ...
      IF EXISTS(
         SELECT *
           FROM Request_Row Rr, Service F
          WHERE Rr.SERV_FILE_NO = F.FILE_NO
            AND Rr.RQST_RQID = @Rqid
            AND Rr.RECD_STAT = '002'
            AND EXISTS(
               SELECT *
                 FROM dbo.VF$All_Expense_Detail(@PrvnCode, @RegnCode, NULL, @RqtpCode, @RqttCode, NULL, NULL, F.BTRF_Code_Dnrm , F.TRFD_Code_Dnrm)
            )
      )
         UPDATE Request
         SET SEND_EXPN = '002'
