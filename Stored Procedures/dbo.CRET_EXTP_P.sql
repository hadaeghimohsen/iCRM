...
BEGIN
   DECLARE @RqtpCode VARCHAR(3)
	...
	DECLARE C$RequestRequester CURSOR FOR
	   SELECT rr.Code FROM Request_Requester rr, Regulation rg
	...
	DECLARE C$ExpenseItem CURSOR FOR
	   SELECT Code, Rqtp_Code, Rqtt_Code FROM Expense_Item
	    WHERE [Type] = '001' 
	      AND ((@EpitCode IS NULL) OR (Code = @EpitCode));
	
	OPEN C$RequestRequester;
	L$NextRqrqRow:
	FETCH NEXT FROM C$RequestRequester INTO @RqrqCode;
	...
	IF @@FETCH_STATUS <> 0
	   ...
	   OPEN C$ExpenseItem;
	   L$NextEpitRow:
	   FETCH NEXT FROM C$ExpenseItem INTO @EpitCode, @RqtpCode, @RqttCode;
